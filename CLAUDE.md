...
- Orchestration prompts + context knobs (history depth, message cap, opening/turn/style prompts, frequency/presence penalties) live in `lib/conductor.ts`; users edit them in the Prompt Lab on `/settings` (localStorage `agent-arena-conductor`), the chat page sends them as `conductor` with each turn, and the server merges/clamps via `resolveConductor()`. Aborted turns (Pause/disconnect mid-stream) are not persisted
- **Prompt format is "merged-run"** (`TurnManager.buildPrompt`): the speaker's own past turns are raw `assistant` messages (no name prefix); each consecutive run of other voices collapses into ONE `user` message of labeled transcript (`Name: content`); topic line seeds the first user block; the turn instruction (plus per-mode seasoning from `MODE_SEASONING`) rides inside the final user message in brackets. Guarantees strict user/assistant alternation for any persona count — with 2 personas it reduces to the classic backrooms format. Do not add extra trailing messages; that breaks alternation on local chat templates
- Anti-repetition: conductor `frequencyPenalty`/`presencePenalty` (defaults 0.4/0.3) are passed to OpenAI-compatible providers and Ollama (which also gets `repeat_penalty: 1.15`); Anthropic/OpenClaw don't receive them. Free conversation is the primary use case — keep default prompts conversational, not debate-flavored
- Empty turns are never persisted: `/api/chat` retries once with a lifted token cap (thinking models can burn the budget on reasoning), then errors
- Messages that look like ASCII art (`lib/ascii-art.ts` heuristic) render in monospace with preserved whitespace — never add output scrubbing/cleaning that could mangle art
- `start_arena.bat` / `stop_arena.bat` launch/kill the dev server (user convention: every web app ships these)
